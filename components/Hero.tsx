    ...
    <section className="relative min-h-screen flex items-center overflow-hidden" id="top">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-16 w-full">
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          <Link
            href="/#kontakti"
            className="inline-flex items-center gap-2 bg-[#EE7E1A] hover:bg-[#d06e14] text-white font-bold px-6 py-3.5 rounded text-base transition-colors"
          >
            Pasūtīt betonu
            <ArrowIcon />
          </Link>
          <Link
            ...
            className="inline-flex items-center gap-2 border border-white/30 text-white hover:bg-white/10 font-semibold px-6 py-3.5 rounded text-base transition-colors"
          >
            <PhoneIcon />
            +371 24 209 209
          </Link>
        ...
        <div className="flex flex-wrap gap-2">
          {["C8/10 → C35/45", "Tērauda & PP fibras", "Sūknis ar 41 m strēli", "Manipulators 25 t"].map((t) => (
            <span
              key={t}
              className="text-xs text-white/60 border border-white/20 px-3 py-1 rounded-full font-mono"
      ...
      {/* Cert badge */}
      <div className="absolute top-24 right-6 hidden lg:flex items-center gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl px-4 py-3">
      ...
      <Link
        href="/#stats"
        className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-white/40 hover:text-white/70 transition-colors text-xs tracking-widest"
        aria-label="Ritināt zemāk"
      >
        <span>Ritināt</span>
